...
;(function($) {
    ohjui['ohjpageslidertouch'] = function(elem, settings) {
        ...
        this.renderScroller = function() {
            ...
            if(!elem.attr('id')) { elem.attr('id',$.fn.uuid()); }
                
            if(elem.children().length == 0) { elem.html('<ul class="content"></ul>') } else { $(elem.children().get(0)).addClass('content'); };
            elem.find('.content').css({'margin':0});
        ...
        this.scrollToBottom = function() {
            if(elem.children('ul').height() > elem.height()) {
                setTimeout(function() { iscroll.scrollTo(0,(elem.children('ul').height()- elem.height()) * -1); } , 0);
            }
        }
